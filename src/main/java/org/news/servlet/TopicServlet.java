    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("del".equals(opr)) { // 删除主题
            ...
            request.getRequestDispatcher("/newspages/topic_list.jsp").forward(
                    request, response);*/
            Topic topic = null;
            StringBuffer topicsJSON = new StringBuffer("[");
            for (int i = 0;;) {
                topic = list.get(i);
                topicsJSON.append("{\"tid\":" + topic.getTid() + ",");
                topicsJSON.append("\"tname\":\"" + topic.getTname() + "\"}");
                if ((++i) == list.size()) {
                    break;
                } else {
                    topicsJSON.append(",");
            ...
            /*
            request.setAttribute("list", list);
            request.getRequestDispatcher("/newspages/showTopics.jsp").forward(
                    request, response);
            */
            Topic topic = null;
            StringBuffer topicslist = new StringBuffer("");
            for (int i = 0; i < list.size(); ++i) {
                topic = list.get(i);
                topicslist.append("<li> &#160;&#160;&#160;&#160; "
                        + topic.getTname() + " &#160;&#160;&#160;&#160; "
                        + "<a href='../newspages/topic_modify.jsp?tid="
                        + topic.getTid() + "&tname=" + topic.getTname() + "'>修改</a>"
                        + " &#160;&#160;&#160;&#160; <a href='../util/topics?opr=del&tid="
                        + topic.getTid() + "'>删除</a> </li>");
            }
            out.print(topicslist);
